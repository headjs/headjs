                ...
                for (var ii = 0, ll = conf.browsers[i][key].length; ii < ll; ii++) {
                    ...
                    if (version > supported) {
                        if (conf.browserCss["gt"])
                            pushClass("gt-" + key + supported);

                        if (conf.browserCss["gte"])
                            pushClass("gte-" + key + supported);
                    }
                    
                    else if (version < supported) {
                        if (conf.browserCss["lt"])
                            pushClass("lt-" + key + supported);

                        if (conf.browserCss["lte"])
                            pushClass("lte-" + key + supported);
                    }
    ...
    function buildRoute() {
        ...
        each(loc.pathname.split("/"), function (el, i) {
            if (this.length > 2 && this[i + 1] !== undefined) {
                if (i) {
                    pushClass(this.slice(1, i + 1).join("-").toLowerCase().replace(/\./g, "-") + conf.section);
                }
            } else {
                // pageId
