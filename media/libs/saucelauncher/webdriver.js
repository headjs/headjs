...
var wd = require('wd'),
    ...
    desired = {
      ...
      "tunnel-identifier": process.env.TRAVIS_JOB_NUMBER ? process.env.TRAVIS_JOB_NUMBER : "headjs",
      "record-video": false
    };
...
browser.init(desired).then(function () {
  ...
  return waitUntilResultsAreAvailable("QUnit.config.stats", 5000);
}).then(function (jsreport) {
  // make an API call to Sauce - set custom-data with 'qunit' data
  var data = {
    'passed': jsreport.bad === 0,
